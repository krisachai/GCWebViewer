...
public class Sources extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            JSONObject jsources = new JSONObject();
             
            GCSource gcsource = new GCSource();
            List<GCSource> sources = gcsource.parseConfig(new Config());
            JSONArray jArray = new JSONArray();
            for (GCSource gCSource : sources) {
                jArray.put(gCSource.getUid());
                
            }
            jsources.put("sources", jArray);
            out.print(jsources);
